...
public class EmpresaControllerTests {
  ...
  */ 
    ...
    void debeActualizarEmpresa() throws Exception{
        Integer id = e1.getId();
        String newWeb = "https://webActualizada.com";
        e1.setWeb(newWeb);

        when(this.usuarioService.obtenerUsuarioActual()).thenReturn(e1.getUsuario());
        when(this.empresaService.obtenerEmpresaPorId(anyInt())).thenReturn(e1);
        when(this.empresaService.actualizarEmpresa(any(Empresa.class), anyInt())).thenReturn(e1);
    ...
    @Test
    void noDebeActualizarEmpresaNoExisteEmpresa() throws Exception{
        Integer id = e1.getId();

        // Usuario autenticado, el empresa con la id proporcionada no existe:
        // debe dar error 404 Not Found
        when(this.usuarioService.obtenerUsuarioActual()).thenReturn(e1.getUsuario());
        when(this.empresaService.obtenerEmpresaPorId(anyInt())).thenThrow(ResourceNotFoundException.class);
        mockMvc.perform(put(String.format(BASE_URL + "/%d", id))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(e1)))
    ...
    @Test
    void noDebeActualizarEmpresaNoLePerteneceEmpresa() throws Exception{
        Integer id = e1.getId();
        
        // Usuario autenticado, el empresa con la id proporcionada no le pertenece:
        // debe dar error 403 Forbidden
        when(this.usuarioService.obtenerUsuarioActual()).thenReturn(e1.getUsuario());
        when(this.empresaService.obtenerEmpresaPorId(anyInt())).thenReturn(e2);
        mockMvc.perform(put(String.format(BASE_URL + "/%d", id))
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    void noDebeActualizarEmpresaErrorDatos() throws Exception{
        ...
        when(this.usuarioService.obtenerUsuarioActual()).thenReturn(e1.getUsuario());
        when(this.empresaService.obtenerEmpresaPorId(anyInt())).thenReturn(e1);
        when(this.empresaService.actualizarEmpresa(any(Empresa.class), anyInt())).thenThrow(new DataAccessException("Error de acceso a datos"){});
        mockMvc.perform(put(String.format(BASE_URL + "/%d", id))
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(e1)))
    ...
    @Test
    void  noDebeBorrarEmpresaNoAutenticado() throws Exception{
        Integer id = e1.getId();
    ...
    @Test
    void noDebeBorrarEmpresaNoExisteEmpresa() throws Exception{
        Integer id = e1.getId();
    ...
    @Test
    void noDebeBorrarEmpresaNoLePerteneceEmpresa() throws Exception{
        Integer id = e1.getId();
    ...
    @Test
    void noDebeBorrarEmpresaErrorDatos() throws Exception{
        Integer id = e1.getId();
        ...
        when(this.usuarioService.obtenerUsuarioActual()).thenReturn(e1.getUsuario());
        when(this.empresaService.obtenerEmpresaPorId(anyInt())).thenReturn(e1);
        doThrow(new DataAccessException("Error de acceso a datos"){}).when(this.empresaService).eliminarEmpresa(id);
        mockMvc.perform(delete(String.format(BASE_URL + "/%d", id)))
                            .andExpect(status().isInternalServerError());
    }
}
